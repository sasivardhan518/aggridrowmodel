...
export class FakeServer {
    ...
    getFirstLevelData(callback, params: IEnterpriseGetRowsParams) {
        ...
        // data['data'] = this.dataGeneratorService.mergeMultipleDataSets(data, data['data'], _.keys(this.outputFormat['columns']));
        ...
        const sub = this.dataGeneratorService
            ...
            .subscribe(dataObj => {
                ...
                // this.formatService.getColumns.emit(this.columnDefs);
    ...
    updateDataForGridRows(parents, parentInfo, group) {
        ...
        this.allLeafsData.forEach((rowGroup: NodeInfo, rowGroupIndex) => {
            ...
            // if (rowGroup.request.groupKeys.length === 0) {
            //     return;
            // }
            ...
            const sub = this.dataGeneratorService
                ...
                .subscribe(dataObj => {
                    ...
                    // setTimeout(function () {
                    //     const lastrow = self.getLastRowResult(rowGroup.data, rowGroup.params);

                    //     rowGroup.params.successCallback(rowGroup.data, lastrow);
                    // }, 3000);

                    // const firstChild = this.gridApi.api.enterpriseRowModel.getRow(rowGroup.node.childrenCache
                    //     .displayIndexStart);
                    // const lastChild = this.gridApi.api.enterpriseRowModel.getRow(rowGroup.node.childrenCache
                    //     .displayIndexEnd - 1);
                    // const allNodes = this.gridApi.api.enterpriseRowModel.getNodesInRangeForSelection(firstChild, lastChild);

                    // for (let i = 0; i < allNodes.length; i++) {
                    //     allNodes[i].updateData(rowGroup.data[i]);
                    // }
                    ...
                    //     rowNodes[i].updateData(rowGroup.data[i]);
                    // }
